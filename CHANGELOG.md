...
* Add ItemsAdder integration: If the ItemsAdder plugin is present, we automatically keep stored ItemsAdder items up-to-date.
  ...
  * Note: Only items that ItemsAdder can positively identify are updated. Legacy items without identifying ItemsAdder data are intentionally left untouched: Guessing their identity, e.g. based on their custom model data, is not safe, because custom model data assignments can shift between resource pack versions.
...
* Fork compatibility: Shop members that were stored by this fork's own previous shop members implementation are loaded with the `full` access level, matching the access those members had before.
...
* Update for MC 26.2.
  * Add sulfur cube to the by default enabled mob types.
    ...
    * Config: Add `sulfur-cube-max-size` (default: `5`). The maximum supported size is `10`.
    * On Spigot, the age lock is currently not working (SPIGOT-8139): The mob spawns with a random age and ages until the next respawn / chunk reload.
...
* Fix: Prevent shopkeeper mobs from being sheared by dispensers. Item drops were already prevented before, but other effects on the entity or the dispensed item were not prevented previously.
* Fix: Since the last plugin version, shopkeepers were always marked as "dirty" every time they are loaded, causing a redundant save of all shopkeeper data.
...
* Update for MC 26.1.2.
  * MC 26.1 and 26.1.1 might work but are not officially supported.
